function CGame(oData, iLevel) {
    ...
    this._init = function (iLevel) {
        ...
        if(_iLevel === 0){
            if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
                new createjs.Tween.get(s_oSoundTrack).to({volume:0.4}, 2000);
            }
        } else {
            if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
                new createjs.Tween.get(s_oSoundTrack).to({volume:0}, 2000);
            }
        }
    ...
    this._onExitHelp = function () {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            new createjs.Tween.get(s_oSoundTrack).to({volume:0}, 1000);
        }
    };
